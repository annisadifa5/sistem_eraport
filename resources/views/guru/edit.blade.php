...
@php
    ...
    $dataGuruOpen = true; 
    $detail = $guru->detailGuru; // Mempermudah akses data detail
@endphp

@section('content')

<div class="p-6 bg-white rounded-lg shadow-md">
    <h2 class="text-2xl font-bold mb-6 text-gray-800">Edit Data Guru: {{ $guru->nama_guru }}</h2>
    
    {{-- Form untuk update data guru. Perhatikan @method('PUT') --}}
    <form action="{{ route('guru.update', $guru->id_guru) }}" method="POST">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
            ...
            <div>
                ...
                <select id="role" name="role" required
                        class="w-full border border-gray-300 rounded-md shadow-sm">
                    @php $role = old('role', $guru->role); @endphp
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
            ...
            <div>
                ...
                <input type="text" name="kewarganegaraan" value="{{ old('kewarganegaraan', $detail->kewarganegaraan ?? '') }}" class="w-full border border-gray-300 rounded-md shadow-sm">
            </div>
            {{-- NIK --}}
            <div>
            ...
            <div>
                ...
                <input type="text" name="no_hp" value="{{ old('no_hp', $detail->no_hp ?? '') }}" class="w-full border border-gray-300 rounded-md shadow-sm">
            </div>
            {{-- Email --}}
            <div class="md:col-span-2">
                <label for="email" class="block text-sm font-medium text-gray-700 mb-1">Email</label>
                <input type="email" name="email" value="{{ old('email', $detail->email ?? '') }}" class="w-full border border-gray-300 rounded-md shadow-sm">
            </div>
        </div>
        ...
        <div class="mt-4 mb-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                <div class="md:col-span-4">
                    ...
                    <textarea name="alamat" rows="3" class="w-full border border-gray-300 rounded-md shadow-sm">{{ old('alamat', $detail->alamat ?? '') }}</textarea>
                </div>
                <div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
            <div>
                ...
                <input type="text" name="status_kepegawaian" value="{{ old('status_kepegawaian', $detail->status_kepegawaian ?? '') }}" class="w-full border border-gray-300 rounded-md shadow-sm">
            </div>
            <div>
                <label for="pangkat_gol" class="block text-sm font-medium text-gray-700 mb-1">Pangkat/Golongan</label>
                <input type="text" name="pangkat_gol" value="{{ old('pangkat_gol', $detail->pangkat_gol ?? '') }}" class="w-full border border-gray-300 rounded-md shadow-sm">
            </div>
            <div>
                <label for="sumber_gaji" class="block text-sm font-medium text-gray-700 mb-1">Sumber Gaji</label>
                <input type="text" name="sumber_gaji" value="{{ old('sumber_gaji', $detail->sumber_gaji ?? '') }}" class="w-full border border-gray-300 rounded-md shadow-sm">
            </div>
            <div>
                <label for="tugas_tambahan" class="block text-sm font-medium text-gray-700 mb-1">Tugas Tambahan</label>
                <input type="text" name="tugas_tambahan" value="{{ old('tugas_tambahan', $detail->tugas_tambahan ?? '') }}" class="w-full border border-gray-300 rounded-md shadow-sm">
            </div>
            {{-- Dokumen --}}
            <div>
                <label for="sk_cpns" class="block text-sm font-medium text-gray-700 mb-1">SK CPNS</label>
                <input type="text" name="sk_cpns" value="{{ old('sk_cpns', $detail->sk_cpns ?? '') }}" class="w-full border border-gray-300 rounded-md shadow-sm">
            </div>
            <div>
                <label for="tgl_cpns" class="block text-sm font-medium text-gray-700 mb-1">Tgl CPNS</label>
                <input type="date" name="tgl_cpns" value="{{ old('tgl_cpns', $detail->tgl_cpns ?? '') }}" class="w-full border border-gray-300 rounded-md shadow-sm">
            </div>
            <div>
                <label for="sk_pengangkatan" class="block text-sm font-medium text-gray-700 mb-1">SK Pengangkatan</label>
                <input type="text" name="sk_pengangkatan" value="{{ old('sk_pengangkatan', $detail->sk_pengangkatan ?? '') }}" class="w-full border border-gray-300 rounded-md shadow-sm">
            </div>
            <div>
                <label for="tmt_pengangkatan" class="block text-sm font-medium text-gray-700 mb-1">TMT Pengangkatan</label>
                <input type="date" name="tmt_pengangkatan" value="{{ old('tmt_pengangkatan', $detail->tmt_pengangkatan ?? '') }}" class="w-full border border-gray-300 rounded-md shadow-sm">
            </div>
            <div>
                <label for="lembaga_pengangkatan" class="block text-sm font-medium text-gray-700 mb-1">Lembaga Pengangkatan</label>
                <input type="text" name="lembaga_pengangkatan" value="{{ old('lembaga_pengangkatan', $detail->lembaga_pengangkatan ?? '') }}" class="w-full border border-gray-300 rounded-md shadow-sm">
            </div>
            <div>
                <label for="tmt_pns" class="block text-sm font-medium text-gray-700 mb-1">TMT PNS</label>
                <input type="date" name="tmt_pns" value="{{ old('tmt_pns', $detail->tmt_pns ?? '') }}" class="w-full border border-gray-300 rounded-md shadow-sm">
            </div>
            <div>
                <label for="karpeg" class="block text-sm font-medium text-gray-700 mb-1">Karpeg</label>
                <input type="text" name="karpeg" value="{{ old('karpeg', $detail->karpeg ?? '') }}" class="w-full border border-gray-300 rounded-md shadow-sm">
            </div>
            <div>
                <label for="karis_karsu" class="block text-sm font-medium text-gray-700 mb-1">Karis/Karsu</label>
                <input type="text" name="karis_karsu" value="{{ old('karis_karsu', $detail->karis_karsu ?? '') }}" class="w-full border border-gray-300 rounded-md shadow-sm">
            </div>
            <div>
                <label for="npwp" class="block text-sm font-medium text-gray-700 mb-1">NPWP</label>
                <input type="text" name="npwp" value="{{ old('npwp', $detail->npwp ?? '') }}" class="w-full border border-gray-300 rounded-md shadow-sm">
            </div>
            <div>
                <label for="nama_wajib_pajak" class="block text-sm font-medium text-gray-700 mb-1">Nama Wajib Pajak</label>
                <input type="text" name="nama_wajib_pajak" value="{{ old('nama_wajib_pajak', $detail->nama_wajib_pajak ?? '') }}" class="w-full border border-gray-300 rounded-md shadow-sm">
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
            <div>
                ...
                <input type="text" name="nama_ibu_kandung" value="{{ old('nama_ibu_kandung', $detail->nama_ibu_kandung ?? '') }}" class="w-full border border-gray-300 rounded-md shadow-sm">
            </div>
            <div>
                <label for="status_perkawinan" class="block text-sm font-medium text-gray-700 mb-1">Status Perkawinan</label>
                <input type="text" name="status_perkawinan" value="{{ old('status_perkawinan', $detail->status_perkawinan ?? '') }}" class="w-full border border-gray-300 rounded-md shadow-sm">
            </div>
            <div>
                <label for="nama_suami_istri" class="block text-sm font-medium text-gray-700 mb-1">Nama Suami/Istri</label>
                <input type="text" name="nama_suami_istri" value="{{ old('nama_suami_istri', $detail->nama_suami_istri ?? '') }}" class="w-full border border-gray-300 rounded-md shadow-sm">
            </div>
            <div>
                <label for="nip_suami_istri" class="block text-sm font-medium text-gray-700 mb-1">NIP Suami/Istri</label>
                <input type="text" name="nip_suami_istri" value="{{ old('nip_suami_istri', $detail->nip_suami_istri ?? '') }}" class="w-full border border-gray-300 rounded-md shadow-sm">
            </div>
            <div>
                <label for="pekerjaan_suami_istri" class="block text-sm font-medium text-gray-700 mb-1">Pekerjaan Suami/Istri</label>
                <input type="text" name="pekerjaan_suami_istri" value="{{ old('pekerjaan_suami_istri', $detail->pekerjaan_suami_istri ?? '') }}" class="w-full border border-gray-300 rounded-md shadow-sm">
            </div>
            {{-- Bank --}}
            <div>
                <label for="bank" class="block text-sm font-medium text-gray-700 mb-1">Nama Bank</label>
                <input type="text" name="bank" value="{{ old('bank', $detail->bank ?? '') }}" class="w-full border border-gray-300 rounded-md shadow-sm">
            </div>
            <div>
                <label for="norek_bank" class="block text-sm font-medium text-gray-700 mb-1">Nomor Rekening</label>
                <input type="text" name="norek_bank" value="{{ old('norek_bank', $detail->norek_bank ?? '') }}" class="w-full border border-gray-300 rounded-md shadow-sm">
            </div>
            <div>
                <label for="nama_rek" class="block text-sm font-medium text-gray-700 mb-1">Nama Rekening</label>
                <input type="text" name="nama_rek" value="{{ old('nama_rek', $detail->nama_rek ?? '') }}" class="w-full border border-gray-300 rounded-md shadow-sm">
            </div>
        </div>
        ...
        <p class="text-sm text-gray-600 mb-4">Hapus atau tambahkan mata pelajaran yang diajarkan oleh guru ini.</p>

        <div class="p-4 bg-gray-50 rounded-lg border border-gray-200">
            <div id="pembelajaran-container-edit">
                
                @php
                    $pembelajarans = old('pembelajaran', $guru->pembelajaran);
                @endphp
                
                @forelse ($pembelajarans as $i => $p)
                @php
                    $id_kelas = is_object($p) ? $p->id_kelas : ($p['id_kelas'] ?? '');
                    $id_mapel = is_object($p) ? $p->id_mapel : ($p['id_mapel'] ?? '');
                @endphp
                <div class="pembelajaran-item grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 mb-4 items-end border-b pb-4">
                    ...
                    <div class="sm:col-span-1 text-right">
                        {{-- Tombol Hapus: disembunyikan jika hanya ada satu baris --}}
                        <button type="button" class="btn-remove-pembelajaran text-red-500 hover:text-red-700 text-sm p-2 {{ $loop->count == 1 && $pembelajarans->count() == 1 ? 'hidden' : '' }}">Hapus</button>
                    </div>
                </div>
                @empty
                {{-- Template kosong jika tidak ada data sama sekali --}}
                <div class="pembelajaran-item grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 mb-4 items-end border-b pb-4">
                    <div class="sm:col-span-1">
                        <label class="block text-sm font-medium text-gray-700 mb-1">Kelas</label>
                        <select name="pembelajaran[0][id_kelas]" class="w-full border border-gray-300 rounded-md shadow-sm">
                            <option value="">Pilih Kelas</option>
                            @foreach ($kelasList as $kelas)
                    ...
                    <div class="sm:col-span-2">
                        <label class="block text-sm font-medium text-gray-700 mb-1">Mata Pelajaran</label>
                        <select name="pembelajaran[0][id_mapel]" class="w-full border border-gray-300 rounded-md shadow-sm">
                            <option value="">Pilih Mapel</option>
                            @foreach ($mapelList as $mapel)
                    ...
                    <div class="sm:col-span-1 text-right">
                        <button type="button" class="btn-remove-pembelajaran text-red-500 hover:text-red-700 text-sm p-2 hidden">Hapus</button>
                    </div>
                </div>
                @endforelse
            </div>
            
            <button type="button" id="btn-add-pembelajaran-edit" 
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        const container = document.getElementById('pembelajaran-container-edit');
        const addButton = document.getElementById('btn-add-pembelajaran-edit');
        
        let index = container.children.length > 0 ? container.children.length : 0; 

        const handleRemove = (e) => {
            e.target.closest('.pembelajaran-item').remove();
        };

        const cloneTemplateEdit = () => {
            const templateHtml = `
                <div class="pembelajaran-item grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 mb-4 items-end border-b pb-4">
                    <div class="sm:col-span-1">
                        <label class="block text-sm font-medium text-gray-700 mb-1">Kelas</label>
        ...
        addButton.addEventListener('click', cloneTemplateEdit);
