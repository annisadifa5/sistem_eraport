...
<head>
    ...
    <title>Input Nilai Sumatif Tengah Semester</title>
    ...
    <div>
        ...
        <div :class="sidebarOpen ? 'flex items-center justify-center py-4 border-b border-blue-500' : 'flex items-center justify-center py-4 border-b border-blue-500'"> 
            <template x-if="sidebarOpen"> 
                <h1 class="text-lg font-semibold">Wali Kelas</h1> 
            </template> 
        ...
        <nav class="mt-6 space-y-1 font-medium">
            ...
            <div>
                ...
                <div x-show="inputNilaiOpen" x-transition class="pl-10 bg-blue-600/40 text-sm">
                    <a href="{{ route('wali.input.tugas') }}" 
                       class="block py-2 flex items-center space-x-2 transition 
                            {{ request()->routeIs('wali.input.tugas') ? 'bg-blue-700/70 rounded-l-md' : 'hover:bg-blue-700' }}">
                        <i class="fa-solid fa-users text-xs"></i><span>Tugas</span>
                    </a>
                    <a href="{{ route('wali.input.ulangan') }}" 
                       class="block py-2 flex items-center space-x-2 transition 
                            {{ request()->routeIs('wali.input.ulangan') ? 'bg-blue-700/70 rounded-l-md' : 'hover:bg-blue-700' }}">
                        <i class="fa-solid fa-users text-xs"></i><span>Ulangan Harian</span>
                    </a>
                    <a href="{{ route('wali.input.sts') }}" 
                       class="block py-2 flex items-center space-x-2 transition 
                            {{ request()->routeIs('wali.input.sts') ? 'bg-blue-700/70 rounded-l-md' : 'hover:bg-blue-700' }}">
                        <i class="fa-solid fa-users text-xs"></i><span>STS</span>
                    </a>
                    <a href="{{ route('wali.input.sas') }}" 
                       class="block py-2 flex items-center space-x-2 transition 
                            {{ request()->routeIs('wali.input.sas') ? 'bg-blue-700/70 rounded-l-md' : 'hover:bg-blue-700' }}">
                        <i class="fa-solid fa-users text-xs"></i><span>SAS</span>
                    </a>
                    <a href="{{ route('wali.input.sat') }}" 
                       class="block py-2 flex items-center space-x-2 transition 
                            {{ request()->routeIs('wali.input.sat') ? 'bg-blue-700/70 rounded-l-md' : 'hover:bg-blue-700' }}">
            ...
            <!-- Cetak Nilai -->
            <a href="{{ route('wali.input.cetak') }}" 
               class="flex items-center py-2 transition
                    {{ request()->routeIs('wali.input.cetak') ? 'bg-blue-800' : 'hover:bg-blue-800' }}"
               :class="sidebarOpen ? 'px-4 justify-start' : 'justify-center'">
        ...
        <div class="bg-white rounded-lg shadow p-6">
            ...
            <div class="flex items-center justify-between border-b pb-3 mb-6">
                <h2 class="text-lg font-semibold text-gray-800 flex items-center gap-2">
                    <i class="text-blue-600"></i> Input Nilai Sumatif Tengah Semester
                </h2>
            </div>
